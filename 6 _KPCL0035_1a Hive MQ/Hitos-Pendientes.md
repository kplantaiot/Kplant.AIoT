...
2.  **Sincronización de Tiempo:**
    ...
    *   **[x] Añadir Timestamp a KPCL0035/SENSORS:** Incluido un timestamp en el payload JSON de los mensajes publicados en el tópico `KPCL0035/SENSORS` en el formato `MM-dd-aaaa HH:mm:ss` para un mejor seguimiento y análisis de datos.
...
4.  **Publicación de Datos de Sensores:**
    *   Lectura y publicación periódica de los datos de los sensores (peso, temperatura, humedad, LDR) en el tópico `KPCL0035/SENSORS`.
...
5.  **Publicación de Estado del Dispositivo:**
    *   Envío periódico de un mensaje de estado (status de WiFi, ID del dispositivo, salud de los sensores) al tópico `KPCL0035/STATUS`.
...
1.  [ ] **Estabilizar Estado del Dispositivo:** El estado `KPCL0035` en el tópico `STATUS` alterna entre "Online" y "Offline" incorrectamente. Se debe investigar y corregir la lógica para que refleje el estado real de la conexión de manera consistente.
...
4.  [x] **Configuración Dinámica del Dispositivo:** Permitir que el `DEVICE_ID` (actualmente "KPCL0035") sea configurable, para facilitar la incorporación de nuevos dispositivos (logrado al usar el `DEVICE_ID` como key en el payload de STATUS).
...
8.  [ ] **Mejorar la estabilidad del peso en modo batería:** Al conectar la placa directo al PC el peso está calibrado y muy estable, pero al pasar al modo batería (completamente autónomo), el peso comienza a incrementar continuamente.
