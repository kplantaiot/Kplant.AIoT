...
static HX711_ADC scale(PIN_HX711_DOUT, PIN_HX711_SCK);
...
void sensorsInit() {
    // La inicialización de la báscula (scale.begin) se hace en el constructor del objeto
    // con la librería HX711_ADC.
...
String sensorsReadAndPublish() {
    ...
    // --- Lectura de Sensores ---
    // HX711 (Peso)
    // Forzamos una lectura nueva y bloqueante para asegurar datos frescos
    if (scale.refreshDataSet()) {
        float weight = scale.getData();
        // Limitar a 3 decimales
        doc["weight"] = roundf(std::max(0.0f, std::min(weight, MAX_WEIGHT_G)) * 1000.0f) / 1000.0f;
    } else {
        sensor_health_status = "ERR_HX711";
        doc["weight"] = nullptr;
        Serial.println("Error: No se pudo leer de HX711.");
    }
    ...
    // LDR (Luz)
    // ADVERTENCIA: PIN_LDR está en un pin ADC2. La lectura puede fallar si se realiza
    // exactamente al mismo tiempo que el WiFi está transmitiendo.
    int ldr_raw = analogRead(PIN_LDR);
    if (ldr_raw < 0) { // -1 o valor negativo indica fallo de lectura en ADC2 con WiFi
        if (sensor_health_status == "OK") {
            sensor_health_status = "ERR_LDR";
        }
        ldr_raw = 0; // Usar 0 como valor de fallback
        Serial.println("Advertencia: Fallo al leer el LDR (conflicto con WiFi?). Usando valor 0.");
    }
